...
AUTH0_DOMAIN = 'learn-auth.us.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'CastingAgencyAPP'
## AuthError Exception
'''
AuthError Exception
...
def get_token_auth_header():
    ...
    if not Authorized:
        ...
        raise AuthError({
                ...
                'description': 'Sorry, Authorization header is expected.'
            }, 401)
    # attempt to split bearer and the token
    token_parts = Authorized.split()

    # raise an AuthError if the header is malformed
    if token_parts[0].lower() != 'bearer':
        raise AuthError({
                'code': 'invalid_header',
                'description': 'Sorry, Authorization header must start with "Bearer".'
            }, 401)

    elif len(token_parts) == 1:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Sorry, Token not found.'
        }, 401)

    elif len(token_parts) > 2:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Sorry, Authorization header must be bearer token.'
        }, 401)
